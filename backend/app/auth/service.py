    ...
    *,
    ...
    if risk.action == "block_temporarily" and risk.cooldown_until:
        ...
        _record_login_attempt(
            ...
            success=False,
            user=user,
            reason=f"cooldown_active:{retry_after}s",
        )
        db.commit()
        raise HTTPException(
    ...
    if risk.action == "challenge":
        ...
        if settings.EMAIL_ENABLED:
            _send_login_challenge_email(user.email, otp_code)
        else:
        ...
        _record_login_attempt(
            ...
            success=False,
            user=user,
            reason=f"security_challenge_required:{risk.level}:{risk.score}",
        )
        db.commit()
    ...
    _record_login_attempt(
        ...
        success=True,
        user=user,
        reason=f"risk:{risk.level}:{risk.score}",
    )
    db.commit()
...
def verify_login_challenge(
    ...
    *,
    ...
    _verify_user_otp(
        db,
        user,
        purpose=OTP_PURPOSE_LOGIN_CHALLENGE,
    ...
    _record_login_attempt(
        ...
        success=True,
        user=user,
        reason="security_challenge_passed",
    )
    db.commit()
...
def _send_login_challenge_email(email: str, otp_code: str) -> None:
    email_service = get_email_service()
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to send login challenge email",
        ) from exc
